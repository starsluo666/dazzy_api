...
def issue_coupon(*, owner, source="manual", issued_by=None, now=None):
    ...
    coupon = UserCoupon.objects.create(
        ...
        source=source,
        issued_by=issued_by,
        face_amount=rules["report_coupon_amount"],
        min_order_amount=rules["report_coupon_min_order_amount"],
        expires_at=now + timedelta(days=rules["report_coupon_valid_days"]),
    )
